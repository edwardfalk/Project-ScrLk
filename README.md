...
Two AI models share the work. The voice model is the dungeon master: it turns what you say into game commands and tells you the result. After every move, a second, smaller model reads what the game answered and writes a description of the scene for the picture. It also decides whether the move visibly changed the room, which is what makes him imagine it again. It had one early misunderstanding. Many of Zork's room descriptions start with what isn't there: *"There is no door here, and all the windows are boarded."* The model took that as the game refusing the move, so it told you that you couldn't go north and then described the room you had just walked into. Now it is told outright whether the move worked.
...
- Tim Anderson, Marc Blank, Bruce Daniels and Dave Lebling, who wrote the original Zork at MIT, and Infocom, where Zork I was released in 1980. The remake keeps their room descriptions.
- [ZorkAI](https://github.com/arsindelve/ZorkAI), a remake of the Infocom classics with an AI narrator, whose architecture the Zork engine is modelled on.
